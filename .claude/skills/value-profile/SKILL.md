...
**Gate 行为**: 三项任一 → `假 / 存疑`, 子 agent 停止深度定性研究, 把该 section 标 `**置信度:** 低`, 并在开头引用处写明失败原因, 不得进入估值。Step 6 遇此 gate 失败需 abort 并打出 "估值前置清单未通过" 提示条, 引用本 gate。
...
- **三大前提 gate**（§3.pre）— §1 / §3 / §5 sections 必需。子 agent 在任何定性段落前先输出 3 行 gate 判定。
...
**前置 gate 检查**: 若 §3.pre 三大前提 gate 的任一项为 `假 / 存疑`, abort Step 6 并打 提示条:
