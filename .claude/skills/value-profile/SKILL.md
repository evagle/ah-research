...
3. **分析框架**（本项目默认):
   - 三大前提（真报表 + 真盈利 + 可持续）—— 任一假/存疑, 立即降级为定性研究, 禁止进入估值
   - 能力圈四问（§1 商业模式前置, 理解门槛）—— 公司卖什么 / 客户为何买它 / 资本为何没抢光蛋糕 / 巨额挑战能否守住; 任一答抽象空话, 退回 §0 重审是否看得懂
...
**判定行为**: 三项任一 → `假 / 存疑`, 子 agent 停止深度定性研究, 把该 section 标 `**置信度:** 低`, 并在开头引用处写明失败原因, 不得进入估值。Step 6 遇此前提不成立需 abort 并打出 "估值前置清单未通过" 提示条, 引用本节。

完整推导 + 案例见 `docs/value-profile/methodology-tang.md` §A.2。

#### 3.pre-1 — §1 商业模式前置条件（能力圈四问）

**与 §3.pre 并列的前置条件**。三大前提看的是**报表真伪 + 可持续性**（估值门槛）; 能力圈四问看的是**研究者本人对商业机理的把握程度**（理解门槛）。两者必须同时成立, 缺一不可。专门用于 §1 商业模式 section worker 的开场白——目的是天然避免 §1 沦为抽象空话（"行业龙头 / 品牌强大 / 成长空间巨大"）。

每次派 §1.x section worker 子 agent 前, 主 agent 先要求子 agent **逐问回答以下四问**, 每问独立一段、每段 ≥ 50 字、必带 ticker 特定证据（产品名 / 工艺 / 客户场景 / 竞品名 / 年报页码）:

1. **公司靠销售什么商品 / 服务获取利润?**
   - 要求: 写清楚具体 SKU / 产品线 / 服务类型 + 收入占比 + 毛利结构。
   - **失败信号**: 回答 < 50 字, 或仅复读公司名 / 行业名（"茅台卖酒" / "腾讯卖互联网服务"）, 或没有产品级拆分 → 未达标。

2. **客户为何从它这里采购, 而不选其他机构?**
   - 要求: 具体到**一个客户场景** + **切换成本** + **竞品对比**。客户为什么不走竞品? 切换代价在哪里（品牌 / 渠道 / 工艺 / 价格带 / 关系 / 合规门槛）?
   - **失败信号**: 只写"品牌强" / "客户粘性高" / "信任度好" 这类结论标签而无具体场景演练 → 未达标。

3. **资本的天性是逐利。为什么别的资本没提供更高性价比, 抢走它的市场份额或逼它降低利润?**
   - 要求: 点名**前 3 家最有资本 / 最有动机的挑战者**（外资巨头 / 同行大厂 / 跨界新进入者 / 国家队）, 说明它们过去 5 年尝试过什么、结果如何（份额涨跌 / 定价动作 / 渠道反应）。
   - **失败信号**: 写"没人敢进" / "壁垒太高"而不指名挑战者、不给 5 年份额数字 → 未达标。

4. **假设同行挟巨资 / 产业巨头挟巨资参与竞争, 该公司能否保住乃至继续扩张市场份额?**
   - 要求: 构造**最坏情景 假想敌**（例: 茅台假想"五粮液集团斥 1000 亿 10 年反攻高端" / 腾讯假想"字节斥 500 亿做微信替代"）, 具体推演该公司哪些资产 / 机制能扛住, 哪些会被侵蚀。
   - **失败信号**: 用"龙头地位稳固"这类空话敷衍, 不做具体假想敌推演 → 未达标。

**判定行为**: 四问任一 `未达标` → 子 agent 不得进入 §1 深挖; 主 agent 退回 §0 重审"这家公司真的看得懂吗 / 是不是能力圈外", 把 §1 全部子 section 标 `**置信度:** 低`, 并在 §1.1 填写区头部标注"能力圈四问第 X 问未达标 — 需补充证据或放弃"。这是**理解门槛**, 不是选择题; 写不出四问的具体答案 = 不下注 = profile 降级为仅定性观察, 不得进入估值。

完整推导见 `docs/value-profile/methodology-tang.md` §I.3。
...
| 三大前提（§3.pre） | 第十节"审计报告" + 第五节现金流量表 + 附注 |
...
- **三大前提**（§3.pre）— §1 / §3 / §5 sections 必需。子 agent 在任何定性段落前先输出 3 行判定结果。
- **能力圈四问**（§3.pre-1）— §1 商业模式所有 subsection（§1.1–§1.7）必需。子 agent 在填写区正文前先用 4 段独立文字答四问, 每段 ≥ 50 字, 含产品 / 客户场景 / 竞品 / 假想敌等 ticker 特定证据。主 agent 复核任一问未达标 → 退回 §0 重审。
...
- §1 商业模式 subsection 的四问答案任一 < 50 字 或 仅品牌名复读（如"茅台卖酒"）或 仅结论标签无具体场景（"客户粘性高"）→ 视为能力圈四问未达标, 不得保存, 退回子 agent 补具体证据。
...
5. 用户确认节点 — 仅 offer `[accept / edit: <text> / research more: <hint>]`。**不 offer `defer` / `skip`**。排雷是强制的, 跳过会让整份 profile 失效。

### Step 6 — 执行摘要合成（Part 0, 估值输出）

触发条件: ≥ 80% 的 section 标 `已完成`（由 Step 2 的 progress map 计算）。

**前置检查**: 若 §3.pre 三大前提的任一项为 `假 / 存疑`, abort Step 6 并打 提示条:
...
2. `yes` → 主 agent 读完所有完成 section, 按以下 **7 字段结构化** 中文输出（取自 `methodology-tang.md` §F）。任一字段缺失 → 该 section 回 `进行中`, 非 `已完成`。
   ...
   2. **合理 PE** = `1 / 当前 10y 国债 收益率`。当前无风险收益率约 3.5% → 合理 PE ≈ 28x（典型区间 25-30; 超出需 justify）。**注意**: 合理 PE 不随增速调整——增速已经反映在 3y 净利润里。
      ...
      **2a. 生意类型估值调整矩阵**（不同商业模式 PE 锚 / 买点折扣 / 三年后净利润可预估度 差异很大, 套用前先判定公司落在哪一类; 标 "不适用 PE" 的 直接 abort Step 6, 回 Part 0 标记 "需 改用 其他 估值法, 无法 走 标准 PE 折现 路径"）:

      | 生意类型 | PE 锚 | 买点折扣 | 三年后净利润可预估度 | 要点 |
      |---|---|---|---|---|
      | 强护城河消费 / 平台互联网龙头 | 25（顶级 30）| 50% | 高 | 茅台 / 腾讯 级别 护城河 + 物理可验证 盈利基础 才 敢 用 30; 常规 取 25, 中值 PE 20 也 可 |
      | 周期股 / 资源 / 航运 / 化工 / 水泥 / 钢铁 | **不适用 PE** | — | 极低 | 顶部 PE 极低 是 陷阱, 底部 PE 极高 是 机会; 改用 "穿越周期平均净利 × 15PE" 或 资产清算 PB < 1, 否则 弃权 |
      | 高成长股（年化 > 25%）| 25 上限 不 破 | 50% | 中 | 三年后 净利润 用 保守 区间 下限; **不准** 用 PEG 抬 PE 到 40-50, 错过 比 错买 好 |
      | 金融股 — 银行 | **改用 PB**（1.0-1.15 × 真实净资产）| 35%（35% 安全边际 即 65% 折扣）| 视 ROE / 坏账 隐含 | 不 用 PE（盈利质量 不可靠, 拨备 可调 利润）; 真实 净资产 = 账面 净资产 − 未 计提 的 不良 真实 损失; 或 用 分红 / 无风险 收益率 近似 永续 债 法, 两 路 取 较 低者 |
      | 金融股 — 保险 | **默认 回避**（内含价值 大 打 折扣）| — | 极低 | 寿险 利润 靠 折现率 / 死亡率 / 退保率 / 投资 收益率 多项 假设 算出, 无 客观 验证 标准; 理性 投资人 要么 远离, 要么 净利 大打 折扣 后 再 估 |
      | 高杠杆企业（房地产 / 部分 电力 / 开发商）| 从 25 下调 到 8-12 | **35%** | 中 | 折扣 从 50% 加深 到 35%, 容错 空间; PE 下移 对应 "1 / (rf + 周期 溢价 + 杠杆 溢价)"; 估值 基础 改 用 "已 锁定 未 结转 收入 对应 净利 + 土储 货值 × 稳态 利润率 × 50% 实现度"; 判定 高杠杆 的 硬 指标: 有息负债 / 净资产 > 1, 或 有息负债 / 经营现金流(近3年) > 3 |
      | 公用事业（水电 / 高速 / 港口 类 "股性 的 债"）| **改用 DCF 简化版** | 股息率 > rf × 1.3 进 买点 区 | 高（运营 稳定 后）| 不走 PE 路径; 用 稳态 自由现金流 / (rf + 2%); 关键 看 折旧 与 维持 性 资本 开支 的 差额 是否 稳定 为 正, 决定 分红 可 持续性 |

      判定 准绳: 公司 同时 符合 多 类 时 (如 高杠杆 + 周期), 用 最 严的 那 一 档。任一 判定 为 "不适用 PE" 或 "默认 回避" → Step 6 不 输出 估值 数字, Part 0 标 "定性 研究 only"。
   ...
   6. **持仓姿态（discrete）**:
      ...
      **估值动摇即停手 守则**（跌破 买点 继续 跌 时 的 硬 规则）: 若 当前 市值 继续 跌 触发 买点 第二 档 / 第三 档 加仓 信号, 但 与 此同时 对 "三年后 净利润 预估" 产生 新的 疑虑 (例: 最新 季报 / 年报 披露 新 信息 动摇 产能 假设、竞争 格局 出现 质变、之前 推导 被 发现 存在 逻辑 漏洞、三大 前提 的 某 一 项 答案 由 "真" 松动 到 "存疑"), **立即 停止 加仓**, 不再 按 原 计划 机械 摊低 成本。正确 的 应对 顺序: ① 回头 重 审 三年后 净利润 下限; ② 重 算 合理 估值 + 新 买点; ③ 再 决定 新的 行动 (继续 加 / 持有 观望 / 止损 重估)。**承认 "我 看 错了" 是 完全 合法 的 结论**, 比 "死守 原 计划 一路 加仓" 更 安全; 下注 行为 正确 不 等于 下注 结果 正确, 估值 判断 本身 已经 动摇 时, 纪律 化 加仓 会 从 "遵从 体系" 滑 向 "赌气 / 摊低 成本 执念"。
...
3. 用户确认节点（`[accept / edit / research more]`）→ save。
...
14. **金融资产 4 分类 必须 拆清 利润表 vs OCI 去向。** 读 附注 "金融 资产" 时, 严格 区分 以下 4 类, 判 其 公允 价值 变动 进 哪张表:
    - **以 摊余 成本 计量**（应收账款、应收票据、贷款 等）— 不 进 利润表, 除非 减值;
    - **以 公允 价值 计量 且 变动 计入 当期 损益**（交易性 金融资产）— **直接 进 利润表**, 是 利润 波动 来源;
    - **以 公允 价值 计量 且 变动 计入 其他 综合 收益（债权 类, 旧称 可 供 出售）**（其他 债权 投资 / 应收款项 融资）— 进 OCI, 最终 卖出 时 累积 盈亏 可 转 入 利润表 → 是 历史 上 公司 调节 利润 的 核心 利器（雅戈尔 5% 门槛 戏法 即 此类）;
    - **以 公允 价值 计量 且 变动 计入 其他 综合 收益（权益 类）**（其他 权益 工具 投资, 新 准则 下 的 "肉粽"）— 从 持有 到 卖出 **永远 不 进 利润表**, 非 交易性 权益 投资 若 被 指定 为 此类 意味 着 买卖 全 部 不 影响 报表 净利, 必须 在 §Q3 / §4.5 里 明写 出 持仓 金额 和 指定 理由。任何 公司 持股 比例 恰好 卡 在 5% / 20% / 50% 分类 门槛 附近, 视为 "会计 分类 戏法" 疑点, 强制 追问 之前 3 年 是否 发生 过 跨档 切换。
15. **合同 负债 vs 预收账款 口径 差异 必 校。** 同行 对比 营收 / 利润率 / 真实 销售 之前, 先 读 附注 "合同 负债" 和 "其他 流动 负债" 披露, 校 两件 事: ① 新 旧 准则 过渡期 的 口径 切换 (2020 年 新 收入 准则 引入 合同 资产 / 合同 负债, 历史 数据 可比 口径 要 自己 合并 "预收款 + 应付 经销商 折扣"); ② 同 行业 不同 公司 做法 差异 — 例: 白酒 里 洋河、五粮液 把 预收款 **含 增值税** 全额 归入 合同负债, 泸州 老窖 把 预收款 **不含 税 部分** 列入 合同负债、增值税 部分 列入 "其他 流动 负债 — 应交 税款", 直接 口径 对比 会 得 错 论。同时 记住 会计 口径 硬 差异: **合同 资产 和 应收账款 都 含 增值税, 合同 负债 不 含 增值税** — 三表 勾稽 和 销售 收现 交叉 验证 时 是 常 踩 的 税款 坑。

## Language policy

- **Profile 内容（`.md` 文件）**: 中文。每个填写区、`**引用:**`、`**管理层口径校核:**`、总结段落均为中文。
- **Operator-facing 输出**（确认节点 prompts、status summary、errors）: 双语。中文为主, 必要时附英文括注。
