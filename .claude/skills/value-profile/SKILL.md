...
3. **分析框架**（本项目默认):
   - 三大前提 gate（真报表 + 真盈利 + 可持续）—— 任一假/存疑, 立即降级为定性研究, 禁止进入估值
...
Skill 不会自行终止, 每个 section gate 后都会把控制权交回用户。
...
**Gate 行为**: 三项任一 → `假 / 存疑`, 子 agent 停止深度定性研究, 把该 section 标 `**置信度:** 低`, 并在开头引用处写明失败原因, 不得进入估值。Step 6 遇此 gate 失败需 abort 并打出 "估值前置 checklist failed" banner, 引用本 gate。
...
| 三大前提 gate（§3.pre） | 第十节"审计报告" + 第五节现金流量表 + 附注 |
...
- **三大前提 gate**（§3.pre）— §1 / §3 / §5 sections 必需。子 agent 在任何定性 prose 前先输出 3 行 gate 判定。
...
3. **管理层 承诺 vs 兑现**（§4 必需）:
   ...
   - §4.3 企业家 checklist 应用言行一致检验, 至少给 2 个具体例子（决策 + 日期 + 结果）。
...
读子 agent 的产出。**Reject and re-dispatch**（回到 3b, 附 refinement hint）若任一:
...
2. `bulk` → 派 ONE `general-purpose` 子 agent:
   ...
   - 对顶行（ROE、毛利率、净利率）用雪球 F10 做 web 交叉检验; 有 discrepancy 当场 inline 报告。
...
### Step 5 — 排雷 checklist mode（§4.5 被选中时触发, Part 4）
...
5. user gate — 仅 offer `[accept / edit: <text> / research more: <hint>]`。**不 offer `defer` / `skip`**。排雷是强制的, 跳过会让整份 profile 失效。

### Step 6 — 执行摘要合成（Part 0, 估值输出）

触发条件: ≥ 80% 的 section 标 `已完成`（由 Step 2 的 progress map 计算）。

**前置 gate 检查**: 若 §3.pre 三大前提 gate 的任一项为 `假 / 存疑`, abort Step 6 并打 banner:
> `❌ 估值前置 checklist failed（三大前提 §<which> = 假/存疑）. 无法进入估值. 请先修复 §3.pre, 或将 Part 0 标记为"不可估值 — 仅定性研究"。`
...
2. `yes` → 主 agent 读完所有完成 section, 按以下 **7 字段结构化** 中文输出（取自 `methodology-tang.md` §F）。任一字段缺失 → 该 section 回 `进行中`, 非 `已完成`。
   ...
   **置信度 aggregate**: `**置信度:** 高` 当 ≥ 60% section 为高 AND §3.pre 三大前提全部 = 真; `中` 混合情况; `低` 若任一块未做 OR 任一前提 = 存疑。

   **Labeling**: 每份 Part 0 draft 必须以这行结尾: `> 本摘要基于 AI 研究 + 用户审阅, 非投资建议. 最近审阅日期 = <today>.`

3. user gate（`[accept / edit / research more]`）→ save。

## 深度调研 playbook（本项目实战积累, 每次新 ticker 研究前过一遍）
...
- **Operator-facing 输出**（gate prompts、status summary、errors）: 双语。中文为主, 必要时附英文括注。
...
| 子 agent quota / rate-limit | 用更窄 page range retry 一次。仍失败 → 存一段 `待补充` + 原因, 状态留 `进行中` |
