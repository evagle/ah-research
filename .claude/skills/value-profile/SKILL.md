...
3. **分析框架**（本项目默认):
   ...
   - 能力圈四问筛（§1 商业模式前置, 理解门槛）—— 公司卖什么 / 客户为何买它 / 资本为何没抢光蛋糕 / 巨额挑战能否守住; 任一答抽象空话, 退回 §0 重审是否看得懂
...
#### 3.pre-1 — §1 商业模式前置筛（能力圈四问）

**与 §3.pre 并列的前置筛**。三大前提筛看的是**报表真伪 + 可持续性**（估值门槛）; 能力圈四问看的是**研究者本人对商业机理的把握程度**（理解门槛）。两者必须同时通过, 缺一不可。专门用于 §1 商业模式 section worker 的开场白——目的是天然避免 §1 沦为抽象空话（"行业龙头 / 品牌强大 / 成长空间巨大"）。
...
1. **公司靠销售什么商品 / 服务获取利润?**
   ...
   - **失败信号**: 回答 < 50 字, 或仅复读公司名 / 行业名（"茅台卖酒" / "腾讯卖互联网服务"）, 或没有产品级拆分 → 未过筛。

2. **客户为何从它这里采购, 而不选其他机构?**
   - 要求: 具体到**一个客户场景** + **切换成本** + **竞品对比**。客户为什么不走竞品? 切换代价在哪里（品牌 / 渠道 / 工艺 / 价格带 / 关系 / 合规门槛）?
   - **失败信号**: 只写"品牌强" / "客户粘性高" / "信任度好" 这类结论标签而无具体场景演练 → 未过筛。
...
3. **资本的天性是逐利。为什么别的资本没提供更高性价比, 抢走它的市场份额或逼它降低利润?**
   ...
   - **失败信号**: 写"没人敢进" / "壁垒太高"而不指名挑战者、不给 5 年份额数字 → 未过筛。
...
4. **假设同行挟巨资 / 产业巨头挟巨资参与竞争, 该公司能否保住乃至继续扩张市场份额?**
   ...
   - **失败信号**: 用"龙头地位稳固"这类空话敷衍, 不做具体假想敌推演 → 未过筛。

**筛查行为**: 四问任一 `未过筛` → 子 agent 不得进入 §1 深挖; 主 agent 退回 §0 重审"这家公司真的看得懂吗 / 是不是能力圈外", 把 §1 全部子 section 标 `**置信度:** 低`, 并在 §1.1 填写区头部标注"能力圈四问第 X 问未过筛 — 需补充证据或放弃"。这是**理解门槛**, 不是选择题; 写不出四问的具体答案 = 不下注 = profile 降级为仅定性观察, 不得进入估值。
...
- **能力圈四问筛**（§3.pre-1）— §1 商业模式所有 subsection（§1.1–§1.7）必需。子 agent 在填写区正文前先用 4 段独立文字答四问, 每段 ≥ 50 字, 含产品 / 客户场景 / 竞品 / 假想敌等 ticker 特定证据。主 agent 复核任一问不过筛 → 退回 §0 重审。
...
- §1 商业模式 subsection 的四问答案任一 < 50 字 或 仅品牌名复读（如"茅台卖酒"）或 仅结论标签无具体场景（"客户粘性高"）→ 视为能力圈四问筛未过, 不得保存, 退回子 agent 补具体证据。
