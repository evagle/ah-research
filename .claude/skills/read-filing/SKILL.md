...
- **限定式公开记录结论**:event manifest是优先的机器复核路径。若调用方已保存可复核的交易所公告枚举、适用监管/执法/司法路线终态和全部命中分类,且没有未解决的正面命中或证据冲突,允许返回`public_record_clearance=qualified`供父skill写`截至AS_OF，适用公开来源未发现其他重大事件`;不得写绝对不存在。缺少数学上完备的全量registry本身不是负面事实或估值阻断。唯一权威路线不可达且无同功能替代、命中无法分类或覆盖范围实质过窄时仍返回未决,不得使用该限定路径。
- **内部资料边界**:采购台账、SKU级返利和独立OEM同规格报价通常不在公开披露范围。事实层应返回已披露金额、条款、余额、治理程序和可得代理证据及明确限制;不得把该限制本身写成价格公允、价格不公允或自动`需人工`。只有正面重大异常无法由公开证据或压力测试评估时才请求父skill升级。
