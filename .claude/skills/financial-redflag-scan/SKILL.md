...
`.claude/skills/value-profile/references/financial-reading.md` — 8 项红旗详解（§4）+ 三表勾稽公式（§5）+ 造假高频模式（§6）+ 必读附注 12 项（§3）。子 skill 派子 agent 前, 主 agent 先 Read 本文件, 并把 §3 / §4 / §5 的阈值具体内化到 subagent prompt 中。
...
资产负债表 / 利润表 / 现金流量表必须互相对得上。任一表的数字不能被另两张表交叉验证 → 疑点。关键公式（见 references/financial-reading.md §5）必须跑完 4 条, 不过的项目直接标 `需人工`。
...
4. **Read reference**: 主 agent Read `.claude/skills/value-profile/references/financial-reading.md` §3 / §4 / §5 把阈值内化。
...
- `.claude/skills/value-profile/references/financial-reading.md` — **必读**, 本 skill 的深度操作手册:
  - §3 必读附注 12 项（对应 29 项清单的附注来源）
  - §4 红旗清单简版（6 项高危模式的阈值来源）
  - §5 三表勾稽 4 条公式（§2.3 的公式来源）
  - §6 造假高频模式（6 类: 收入端 / 成本端 / 现金端 / 利润端 / 结构端）
  - §9 特殊场景加读清单（商誉 > 20% / 有息负债 > 净资产 / 金融资产 > 营收 等）
- `.claude/skills/value-profile/references/moat-framework.md` — 了解生意模型以判断某些红旗的严重性（例: 消费品行业商誉 20% 严重, 周期行业可能只是并购周期）

派子 agent 时, 若需更深操作手册, 在 prompt 里明确告知 "reference 路径 = `.claude/skills/value-profile/references/financial-reading.md` 第 N 节"。
