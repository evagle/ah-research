...
- **§2.3.3关联交易**: 读附注"关联方及关联交易"; 特别关注关联采购占成本比/关联销售占营收比/关联担保/大股东占款; 关联交易定价是否公允。采购台账或向独立OEM询价通常属于公司内部资料。完成公开来源调查后,不得仅因公开资料无法取得而标`需人工`或阻断估值;基于已披露交易金额、条款、期末余额、治理程序和可得代理证据作限定判断,并明确不得据此反向证明关联采购价格公允。仅交易规模或集中度高时写`有异常迹象/预警`并通过gate;只有出现异常定价、资金转移或无法解释的现金流异常,且重大影响无法用公开证据或压力测试评估时才转`需人工/待定`。
...
- **§2.7.1c公开记录限定通过**:严格event manifest仍是优先的机器复核路径;但发行人上市以来交易所公告已枚举,适用监管、执法和司法公开路线已terminal,全部正面命中已归类,且没有未解决的正面命中或证据冲突时,允许写公开记录限定通过:`截至AS_OF，适用公开来源未发现其他重大事件`。该结论不得表述为绝对不存在。缺少数学上完备的全量registry或完整事件清单本身不构成估值阻断。若唯一权威来源不可达且无同功能替代、已知事件可能触发否决但无法分类,或覆盖范围实质过窄,仍写`需人工/待定`。
