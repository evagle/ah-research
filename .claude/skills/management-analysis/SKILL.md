...
一流生意 + 三流管理层 通常优于 三流生意 + 一流管理层（因为好生意的经济商誉能让平庸管理层也挣到钱）。但 §4 道德风险（虚假陈述 / 处罚 / 股东利益输送 / 系统性画大饼）**一票否决**——即使生意再好也 abstain。
...
- **§2.7.2 后果**: 直接大幅降级, Mode A 报告结论 "abstain"; Mode B 回填主 profile §4 时 `**置信度:** 低` + 主 profile §4.5 排雷同步 flag + Part 0 标 "管理层道德风险, 不可估值"
...
- **监事会报告必读**（§2.6）: 引用非空话段落
- **道德风险 overlay**（§2.7）: 虚假陈述 / 处罚 / 关联交易非公允 / 大股东占款 / 系统性画大饼
...
- 若 verdict = abstain, 额外加 `> ⚠️ 管理层道德风险: <描述>. 建议主 value-profile 扫描时同步标 Part 0 "管理层风险, 不可估值".`
